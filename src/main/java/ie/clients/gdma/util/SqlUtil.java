...
public class SqlUtil {
            ...
            for (Filter filter : filters) {
                ...
                if (filter.isValid()) {
                    ...
                    if (StringUtils.hasText(filter.getFilterValue())) {
                        ...
                        stringBuilder.append("upper(cast(");
                        stringBuilder.append(table.getName());
                        stringBuilder.append('.');
                        stringBuilder.append(filter.getColumnName());
                        stringBuilder.append(" as char(100)))");

                        stringBuilder.append(" LIKE '%");
                        stringBuilder.append(filter.getFilterValue().trim().toUpperCase());
                        stringBuilder.append("%'");

                        // stringBuilder.append(table.getName());
                        // stringBuilder.append('.');
                        // stringBuilder.append(filter.getColumnName());
                        // if (isNumeric(filter.getColumnType())) {
                        // stringBuilder.append(" = ");
                        // stringBuilder.append(filter.getFilterValue().trim());
                        // stringBuilder.append(" ");
                        // } else if (isDate(filter.getColumnType())) {
                        // // FIXMe
                        // stringBuilder.append(" = '");
                        // stringBuilder.append(filter.getFilterValue().trim());
                        // stringBuilder.append("' ");
    ...
    public static boolean isNumeric(String sqlDataType) {
        ...
        if (StringUtils.hasText(sqlDataType)) {
            blnReturn = "NUMERIC".equals(sqlDataType) || "TINYINT".equals(sqlDataType) || "INTEGER".equals(sqlDataType)
                    || "BIT".equals(sqlDataType) || "SMALLINT".equals(sqlDataType) || "BIGINT".equals(sqlDataType)
                    || "DECIMAL".equals(sqlDataType) || "FLOAT".equals(sqlDataType) || "REAL".equals(sqlDataType)
                    || "DOUBLE".equals(sqlDataType);
        }
        return blnReturn;
    }
    ...
    public static boolean isText(String sqlDataType) {
        boolean blnReturn = false;
        if (StringUtils.hasText(sqlDataType)) {
            blnReturn = sqlDataType.startsWith("CHAR") || sqlDataType.startsWith("VARCHAR")
                    || sqlDataType.startsWith("LONGVARCHAR");
        }
        return blnReturn;
    }
